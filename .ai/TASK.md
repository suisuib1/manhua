...
- 去掉 diary 页面照片贴纸卡片底部的粉色删除按钮。
- 删除对应的 photo-remove 样式。
- 保留照片展示、添加照片、本地占位和生成按钮交互。
- 未修改其他页面。
...
本轮使用 diagnose skill。根据用户截图，定位 create 页漫画页数和情绪 / 风格标签按钮在小屏预览中贴边、选中态边框造成视觉溢出。

## 本轮处理内容

- 将漫画页数选项改为两列自适应网格。
- 为页数和标签按钮增加透明默认边框，避免选中态边框撑开元素。
- 收紧 create 表单卡片和标签按钮间距，保证按钮不超出卡片和屏幕。
- 未修改其他页面。
...
- 仅修复 diary 页面顶部重复标题、旧装饰残留和底部遮挡问题。
- 未修改其他页面。
- 移除页面内容区重复的“填写日记”标题、旧返回箭头和顶部旧装饰层。
- 保留系统导航栏标题“填写日记”，页面内容顶部只保留一行说明文案。
- 将 diary 根容器保持为 100% 宽度、100vh 最小高度、暖白背景和 240rpx 底部留白。
...
- 调整底部主按钮为普通文档流展示，确保“生成前检查”和按钮完整显示。
