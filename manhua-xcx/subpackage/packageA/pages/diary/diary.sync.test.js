...
function loadPage(storage = {}, requestImpl = () => {}) {
  ...
  global.wx = {
    ...
    navigateTo(options) {
      ...
      if (navigateFailImpl && options.fail) {
        options.fail(navigateFailImpl)
      }
    },
    showModal(options) {
  ...
  return {
    ...
    modalCalls,
    storage,
    setNavigateFail(error) {
      navigateFailImpl = error
    },
  }
}
...
test('diary page prompts for character profile before first generation', () => {
  ...
  assert.equal(modalCalls[0].cancelText, '跳过')
  assert.equal(navigateCalls.length, 0)
  assert.equal(storage.draftComicChapter, undefined)
})

test('empty diary shows toast and does not navigate', () => {
  const { pageConfig, navigateCalls, modalCalls, toastCalls, storage } = loadPage()

  pageConfig.goGenerating()

  assert.equal(toastCalls.length, 1)
  assert.equal(toastCalls[0].title, '请先填写日记内容')
  assert.equal(modalCalls.length, 0)
  assert.equal(navigateCalls.length, 0)
  assert.equal(storage.draftComicChapter, undefined)
})

test('character prompt failure gives feedback instead of doing nothing', () => {
  const { pageConfig, modalCalls, toastCalls } = loadPage()
  ...
  pageConfig.goGenerating()
  modalCalls[0].fail({ errMsg: 'showModal:fail cancelText length invalid' })
...
test('diary page can skip character profile prompt and continue generation', async () => {
  ...
  assert.equal(navigateCalls[0].url, '/subpackage/packageA/pages/generating/generating')
})

test('navigate failure gives feedback', async () => {
  const storage = {
    characterProfile: {
      nickname: '小满',
      personalityText: '温柔',
  ...
  const { pageConfig, setNavigateFail, toastCalls } = loadPage(storage)

  setNavigateFail({ errMsg: 'navigateTo:fail page not found' })
  pageConfig.setData({
    createDraft: {
      chapterTitle: '跳转失败',
    },
  ...
  assert.equal(toastCalls.length, 1)
  assert.equal(toastCalls[0].title, '生成页打开失败，请稍后重试')
})
...
test('diary page character prompt can navigate to character page', () => {
  ...
  assert.deepEqual(navigateCalls[0], {
    url: '/subpackage/packageA/pages/character/character?from=diary',
  })
})
